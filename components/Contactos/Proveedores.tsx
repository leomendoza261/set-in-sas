...
export default function TablaProveedores() {
  const [proveedores, setUsuarios] = useState<Proveedor[]>([]);
  ...
  const usuariosFiltrados = proveedores.filter((usuario) =>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {usuariosFiltrados.map((usuario) => (
              <tr key={usuario.id_proveedor}>
                <td className="border px-2 py-1 text-center">{usuario.id_proveedor}</td>
